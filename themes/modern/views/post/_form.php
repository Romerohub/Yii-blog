...
<div class="popup__overlay">
    ...
    <form action="/index.php/images/addajax" method="POST" enctype="multipart/form-data">
         ...
         <input type="hidden" value="<?php if(!empty($model->id)) echo $model->id; ?>" name="post_id" id="post_id" />
          <input type="hidden" value="post" id="note_type" />
    ...
    <div class="img-container-1" id="img-container-present">
        <ul class="img-list" id="img-list-present">
   		...
   		if(!empty($model->id) or !empty($_POST['Post']['id'])){
   			$img_item = Images::items($model->id);
...
<?php $form=$this->beginWidget('CActiveForm'); ?>
	...
	<?php if(empty($_POST['Post']['id']))echo $form->HiddenField($model,'id'); elseif(!empty($_POST['Post']['id']))
	echo CHtml::HiddenField( 'Post[id]', $_POST['Post']['id']);
	?>
...
<?if(!empty($model->main_pic)){
	if(empty($model->create_time)) $pic_time = time(); else $pic_time = $model->create_time;
	
	?><a target="_blank" href="/uploads/images/<?=date('Y',$pic_time)."/".date('m',$pic_time)."/".$model->main_pic;?>">
	<img width="85px" src="/uploads/images/<?=date('Y',$pic_time)."/".date('m',$pic_time)."/".$model->main_pic;?>">
	</a>
	<?
}?>
</div>
