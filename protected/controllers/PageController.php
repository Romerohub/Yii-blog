 ...
 * */
	protected function loadPostScripts(){
		...
		$cs->registerCssFile($baseUrl.'/css/upload_image.css');
		$cs->registerCssFile($baseUrl.'/css/admin/page/edit.css');
		$cs->registerScriptFile($baseUrl.'/js/lib/jquery.damnUploader.js');
		$cs->registerScriptFile($baseUrl.'/js/admin/upload_image.js');
		$cs->registerScriptFile($baseUrl.'/js/admin/page/edit.js');
	...
	public function loadModel()
	{
		...
		if($this->_model===null)
		{
			if(isset($_GET['id']) or isset($_POST['Page']['id']))
			{
				if(isset($_GET['id'])) $p_id = $_GET['id']; elseif(isset($_POST['Page']['id'])) $p_id = $_POST['Page']['id'];
				if(Yii::app()->user->isGuest)
					$condition='status='.Page::STATUS_PUBLISHED.' OR status='.Page::STATUS_ARCHIVED;
				else
					$condition='';
				$this->_model=Page::model()->findByPk($p_id, $condition);
			}
			if(isset($_GET['url'])){
				if(Yii::app()->user->isGuest)
					$condition='status='.Page::STATUS_PUBLISHED.' OR status='.Page::STATUS_ARCHIVED;
					...
					$condition='';
				$this->_model=Page::model()->findByAttributes(array('url'=>$_GET['url']), $condition);
			}
			if($this->_model===null)
