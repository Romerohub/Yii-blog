$(document).ready(function() {
    ...
    fileInput.damnUploader({
        ...
        onAllComplete: function() {
           // log('<span style="color: blue;">*** Все загрузки завершены! ***</span>');
            ...
            imgSize = 0;
            updateInfo();
            clickByImage(); //'это уже я
            addMainPic(); //'это уже я
            imgList.text('');
        }
    });
    ...
    function addFileToQueue(file) {
        ...
        var title = $('<div/>').html(file.name+' <br><a class="addMainPic" href="#">Сделать главной</a> ').appendTo(li);
        ...
        var uploadItem = {
            ...
            onComplete: function(successfully, data, errorCode) {
                if(successfully) {
                	...
                	//console.log(data);
                	//current_img.attr('src', data);
                	var imgListPres = $('#img-list-present');
                    var li = $('<li/>').appendTo(imgListPres);
                    var title = $('<div/>').html(file.name+
                    		' <br><a class="addMainPic" href="#">Сделать главной</a> ').appendTo(li);
                    var imgPres = $('<img/>').appendTo(li);
                    imgPres.attr('width', 85);
                    imgPres.attr('src', data);
                	//console.log(this);
                } else {
                    if(!this.cancelled) {
    ...
    $("#upload-all").click(function() {
    	fileInput.damnUploader( 'setParam',{
		    url: '/index.php/images/addajax?'+note_type+'_id='+post_id,
        	});
